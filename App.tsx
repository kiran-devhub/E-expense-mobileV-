...
const BasicAddForm = () => {
    const { addTransaction, accounts, createAccount, removeAccount } = useApp();
    ...
    // Ensure Destination is not same as Source
    useEffect(() => {
        if (type === 'TRANSFER' && selectedAccountId === toAccountId) {
            const nextValid = accounts.find(a => a.id !== selectedAccountId);
            if (nextValid) setToAccountId(nextValid.id);
            else setToAccountId('');
        }
    }, [selectedAccountId, type, accounts, toAccountId]);
    ...
    const handleCreateAccount = (e: React.FormEvent) => {
        ...
        if (newAccName && newAccBalance) {
            ...
            setNewAccName('');
            setNewAccBalance('');
            setNewAccType(AccountType.BANK);
    ...
    const handleDeleteAccount = (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (window.confirm("Are you sure you want to delete this account? Transactions linked to it will remain but may have broken references.")) {
            removeAccount(id);
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-soft dark:shadow-none dark:border dark:border-slate-800 space-y-6 relative">
            ...
            <div>
               ...
               <div 
                   ...
                   className="flex gap-3 overflow-x-auto no-scrollbar pb-2 cursor-grab active:cursor-grabbing items-stretch"
               >
                   {accounts.map(acc => (
                       <button
                           key={acc.id}
                           ...
                           className={`relative min-w-[140px] p-4 rounded-2xl border text-left transition-all shrink-0 group ${
                       ...
                       >
                           ...
                           <div 
                                ...
                                className={`absolute top-1 right-1 p-1.5 rounded-lg z-20 transition-all ${selectedAccountId === acc.id ? 'text-white/70 hover:text-white hover:bg-white/20' : 'text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20'}`}
                   ...
                   <button
                        ...
                        className="min-w-[140px] p-4 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center gap-2 text-slate-400 hover:text-indigo-500 hover:border-indigo-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shrink-0"
                   >
                   ...
                   >
                       {accounts
                           ...
                           >
                               ...
                               <div 
                                    onClick={(e) => handleDeleteAccount(acc.id, e)}
                                    className={`absolute top-1 right-1 p-1.5 rounded-lg z-20 transition-all ${toAccountId === acc.id ? 'text-white/70 hover:text-white hover:bg-white/20' : 'text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20'}`}
                                    title="Delete Account"
                               >
                                    <Trash2 size={14} />
                               </div>
            ...
            {showAddAccountModal && (
                <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-3xl animate-in fade-in">
                    <div className="bg-white dark:bg-slate-900 w-full p-6 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-bold text-slate-800 dark:text-white">New Account</h3>
                            <button type="button" onClick={() => setShowAddAccountModal(false)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full p-3 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white font-bold outline-none border border-slate-200 dark:border-slate-700 focus:border-indigo-500" 
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {[AccountType.BANK, AccountType.UPI, AccountType.CASH, AccountType.WALLET, AccountType.CREDIT_CARD, AccountType.DEBIT_CARD].map(type => (
                                        <button
                                            ...
                                            className={`p-2 rounded-lg text-[9px] font-bold border transition-colors truncate ${newAccType === type ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-slate-200 dark:border-slate-700 text-slate-500'}`}
                                        >
                                            {type.replace('_', ' ')}
                                        </button>
                                    ))}
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Balance</label>
                                <input 
                                    type="number" 
                                    value={newAccBalance} 
